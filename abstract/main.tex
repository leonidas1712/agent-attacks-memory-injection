...
We developed a two-turn evaluation framework using InspectAI~\cite{inspect2024} with a custom memory system to simulate persistent agent memory.\footnote{Code and data available at \url{https://github.com/leonidas1712/agent-attacks-memory-injection}}
...
\begin{figure}[t]
  ...
  \caption{Harm rate and stealth score for three Chinese-provider models by injection
...
\textbf{Limitations.} Our memory abstraction (a key--value store serialized into the system prompt) may not capture all production dynamics such as RAG-based retrieval or ranking heuristics. Empty verification-turn responses were conservatively scored as non-harmful, so reported harm rates may be lower bounds. The Chinese-provider model evaluation covers only the Backdoor condition, and we do not implement or test defensive mitigations. Future work should address multi-session persistence, attack transferability across memory architectures, and defense evaluation.
